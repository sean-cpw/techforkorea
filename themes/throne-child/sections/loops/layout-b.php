...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php endif;?>
		...
		<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php esc_attr(get_the_title()); ?>" <?php if(get_post_custom_values('external_link')) echo 'target=\"_blank\"'?>><?php echo thr_get_title('layout-b'); ?></a></h2>
		
		<?php if($meta = thr_child_get_meta_data('layout-b')): ?>
	...
	<div class="entry-content">
		...
		<?php if( $excerpt = thr_get_excerpt('layout-b')) : ?>
		<p>
			<?php echo $excerpt; ?>
			<?php if(thr_get_option('lay_b_rm')) : ?>
			<span class="meta-item">
				<a href="<?php echo esc_url(get_permalink()); ?>" class="read_more_button" <?php if(get_post_custom_values('external_link')) echo 'target=\"_blank\"'?>><i class="icon-arrow-right"></i><?php echo __thr('read_more'); ?></a>
			</span>
			<?php endif; ?>
		</p> 
		<?php endif; ?>
