   ...
   <?php if($i == 1) : $img_size = $img_lyt[$top]; ?>
	 ...
	 <?php endif; ?>				
				...
				?>
					...
					<?php endif; ?>
					<?php if($hover_effect): ?>
						<a href="<?php echo esc_url(get_permalink()); ?>" class="f_overlay" <?php if(get_post_custom_values('external_link')) echo 'target=\"_blank\"'?>></a>
					<?php endif; ?>
					<div class="featured_title_over">
						
						<?php if($meta = thr_child_get_meta_data('fa')) : ?>
							<?php echo $meta; ?>
						<?php endif; ?>
						<h2><a href="<?php echo esc_url(get_permalink()); ?>" title="<?php esc_attr(get_the_title()); ?>" <?php if(get_post_custom_values('external_link')) echo 'target=\"_blank\"'?>><?php echo thr_get_title('fa'); ?></a></h2>
						<?php if($hover_effect): ?>
						<div class="featured_excerpt">
							<p><?php echo thr_get_excerpt('fa'); ?></p>
							<?php if($rm): ?>
								<a href="<?php echo esc_url(get_permalink()); ?>" class="f_read_more" <?php if(get_post_custom_values('external_link')) echo 'target=\"_blank\"'?>><i class="icon-arrow-right"></i> <?php echo __thr('read_more');?></a>
							<?php endif; ?>
						</div>
						<?php endif; ?>
					</div>
